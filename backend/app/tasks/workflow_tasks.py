...
from app.models.workflow import WorkflowExecution, ExecutionLog
...
def execute_workflow_task(self, workflow_id: int, execution_id: int, workflow_data: Dict[str, Any], input_data: Dict[str, Any] = None):
    ...
    try:
        ...
        # Log start
        log_execution(
            db, execution_id, "workflow", "workflow",
            "info", f"Starting workflow execution {execution_id}"
        )
        ...
        result = loop.run_until_complete(
            orchestrator.execute_workflow(workflow_data, input_data)
        )
        loop.close()
        ...
        log_execution(
            db, execution_id, "workflow", "workflow",
            "info", f"Workflow execution {execution_id} {result['status']}"
        )
