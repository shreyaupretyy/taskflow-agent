    ...
    try:
        ...
        if not results:
            return ModelComparisonResponse(
                models=[],
                winner="N/A",
                reason="No execution data available for comparison"
            )
...
async def get_model_metrics(
    ...
    try:
        ...
        avg_rating = sum(e.user_rating for e in rated_executions) / len(rated_executions) if rated_executions else None
        ...
        return {
            ...
            "user_rating": {
                "average": avg_rating,
                "total_ratings": len(rated_executions)
            },
...
async def get_available_models(
    ...
    try:
        models = db.query(AgentExecution.model_name).filter(
            AgentExecution.user_id == current_user.id
        ).distinct().all()
